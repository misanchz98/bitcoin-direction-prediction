...
def analizar_variables_categoricas(datos):
    """
    Analiza variables categóricas (tipo object) en un DataFrame.

    Args:
        datos (DataFrame): El DataFrame que contiene los datos.

    Returns:
        dict: Un diccionario con las diferentes categorías, sus frecuencias absolutas y relativas.
    """
    ...
    for categoria in categoricas:
        resumen = pd.DataFrame({
            'n': datos[categoria].value_counts(),  # Frecuencia absoluta
            '%': datos[categoria].value_counts(normalize=True)  # Frecuencia relativa
        })
...
def patron_perdidos(datos_input):
    ...
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', fmt=".2f", cbar=False, mask=mask, annot_kws={'size': 8})
    
    # Establezco el título del gráfico
    plt.title("Matriz de correlación de valores ausentes")
    plt.xticks(fontsize=8)  # Cambiar tamaño de los ticks del eje x
    plt.yticks(fontsize=8)  # Cambiar tamaño de los ticks del eje y
    plt.show()
